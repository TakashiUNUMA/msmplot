...
if test -s "temp.bin" ; then
    ...
    # for pressure data
    for file in brn cape cin ehi ki lcl lfc lnb pw srh ssi tt wsh lsta ; do
...
if test ${debug_level} -ge 100 ; then
    ...
    mv press.png qv.png temp.png thetae.png cape.png cin.png lcl.png lfc.png lnb.png td.png ki.png pw.png tt.png ehi.png srh.png ssi.png brn.png qv1000.png hgt???.png temp???.png qv???.png thetae???.png wspd???.png pv???.png qfwind???.png qfdiv???.png wsh.png lsta.png ${wwwdir}/autoplot/jmamsm/
    ...
    rm -f press.grd prmsl.grd thetae.grd qv.grd temp.grd [uv]10.grd rh.grd cape.grd cin.grd lcl.grd lfc.grd lnb.grd td.grd temp???.grd qv1000.grd qv???.grd thetae???.grd wspd???.grd [uv]1000.grd [uv]???.grd pv???.grd tt.grd pw.grd ki.grd ehi.grd srh.grd ssi.grd brn.grd qf[uv]???.grd qfwind???.grd qfdiv???.grd hgt???.grd wsh.grd lsta.grd
fi

rm -f press.ps prmsl.ps thetae.ps qv.ps temp.ps cape.ps cin.ps lfc.ps lcl.ps lnb.ps td.ps temp???.ps qv1000.ps qv???.ps thetae???.ps wspd???.ps [uv]???.ps pv???.ps pw.ps tt.ps ki.ps ehi.ps srh.ps ssi.ps brn.ps qfwind???.ps qfdiv???.ps hgt???.ps wsh.ps lsta.ps
rm -f MSM_S
rm -f MSM_P
rm -f cpalet_*
rm -f .gmt*
